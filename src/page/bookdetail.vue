<template>
  <div class="bookdetail">
    ...
    <div class="book-info container-padding">
      <div class="detail">
        ...
        <div class="info">
          ...
          <div class="meta type">
            {{bookData.type}}/{{bookData.serialize}}
          </div>
          <div class="meta num">
            {{bookData.wordcount}}万字
          </div>
        </div>          
      </div>
      <div class="btn">
        <router-link class="active" :to="{path:'/reader/'+ bookData.id}">开始阅读</router-link>
        <router-link :to="{path:'/reader/'+ bookData.id}">加入书架</router-link>
      </div>
      <div class="dec">
        {{bookData.intro}}
      </div>
    ...
    <div class="likes container-padding">
      ...
      <div class="list-item">
        <like-book class="item" :like="value" v-for="(value,index) in likeBooks" key="value"></like-book>
      </div>
    </div>
...
export default {
  ...
  mounted(){
    axios(`${this.apis.api}/booklist?id=${this.$route.params.id}`)
    .then(res => {
      this.loading = true;
    ...
    .nav-title{
      ...
      .goback{
        ...
        span{
          ...
          color: #ed424b;
          background:url('../assets/image/back.png') no-repeat -10% 45%;
          padding-left: .15rem;
