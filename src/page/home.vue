<template>
  <div class="home">
    ...
    <div class="home-nav">
      <a class="book-type-item" href="" title="" v-for="item in bookType" key="item.num">
        <i></i>
        <h4>{{item.name}}</h4>
      </a>
    </div>
    <div class="book-classfiy">
      <div class="" v-if="loading">
        <recommend-list title="热门小说" :bookList="bookList | hot"></recommend-list>
        <recommend-list title="排行榜" :bookList="bookList | top"></recommend-list>
        <hot-list title="新书抢鲜" :bookList="bookList | free"></hot-list>
        <hot-list title="畅销完本" :bookList="bookList | free"></hot-list>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted(){
    axios(`${this.apis.api}/booklist`)
    .then(res => {
      this.loading = true;
      this.bookList = res.data.slice(0,20)
